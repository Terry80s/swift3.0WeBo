...
extension EmoticonController {
    ...
    private func prepareForCollectionView() {
        collectionView.register(EmotionCollectionViewCell.self, forCellWithReuseIdentifier: EmoticonCell)
    ...
    private func prepareForToolBar() {
        ...
        for title in titles {
            let item = UIBarButtonItem(title: title, style: .plain, target: self, action: #selector(EmoticonController.itemClick(item:)))
            item.tag = index
            index += 1
...
extension EmoticonController : UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        // 1.创建cell
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: EmoticonCell, for: indexPath as IndexPath) as! EmotionCollectionViewCell
